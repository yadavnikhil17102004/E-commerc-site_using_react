import p5_img from "./Pet_pro_5.jpg";
import p6_img from "./Pet_pro_6.jpg";
import p7_img from "./Pet_pro_7.jpg";
import p8_img from "./Pet_pro_8.jpg";
import p9_img from "./Pet_pro_9.jpg";
import p10_img from "./Pet_pro_10.jpg";
import p11_img from "./Pet_pro_11.jpg";
import p12_img from "./Pet_pro_12.jpg";

let new_collections = [
  {
    id: 5,
    name: "Ember: Fiery orange tabby with striking stripes, has a playful personality and enjoys chasing laser pointers.",
    category: "cats",
    image: p5_img,
    new_price: 85.0,
    old_price: 120.5,
    age: 2
  },
  {
    id: 6,
    name: "Jasper: Handsome brown tabby with bold stripes, enjoys basking in the sun and chasing after crinkly balls.",
    category: "cats",
    image: p6_img,
    new_price: 85.0,
    old_price: 120.5,
    age: 3
  },
  {
    id: 7,
    name: "Leo: Regal orange tabby with vibrant stripes, loves sunbathing by the window and receiving chin scratches.",
    category: "cats",
    image: p7_img,
    new_price: 85.0,
    old_price: 120.5,
    age: 1
  },
  {
    id: 8,
    name: "Max: Handsome brown tabby with thick stripes, loves exploring outdoor gardens and cuddling up for afternoon naps.",
    category: "cats",
    image: p8_img,
    new_price: 85.0,
    old_price: 120.5,
    age: 4
  },
  {
    id: 9,
    name: "Whisper: Delicate grey tabby with faint stripes, prefers quiet corners and gentle Pet_pros, a shy soul longing for a patient and loving home.",
    category: "cats",
    image: p9_img,
    new_price: 85.0,
    old_price: 120.5,
    age: 2
  },
  {
    id: 10,
    name: "Sunny: Cheerful orange tabby with bright stripes, always wearing a smile and eager to play, spreads joy wherever they go.",
    category: "cats",
    image: p10_img,
    new_price: 85.0,
    old_price: 120.5,
    age: 3
  },
  {
    id: 11,
    name: "Fluffy: Endearing white Persian with a carefree disposition, sporting a perPet_proually ruffled coat and an irresistible charm.",
    category: "cats",
    image: p11_img,
    new_price: 85.0,
    old_price: 120.5,
    age: 1
  },
  {
    id: 12,
    name: "Rocky: Striking black and silver tabby with a confident stance and piercing eyes, exuding an air of strength and independence.",
    category: "cats",
    image: p12_img,
    new_price: 85.0,
    old_price: 120.5,
    age: 4
  },
];

export default new_collections;
